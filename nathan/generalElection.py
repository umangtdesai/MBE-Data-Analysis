...
class generalElection(dml.Algorithm):
    contributor = 'nathan'
    reads = []
    writes = ['nathan.aggGeneralData']
    ...
    def getAggByTownResponse(response):
        ...
        df = pd.read_csv(data)
        df = generalElection.projectGeneral(df)
    ...
    def execute(trial=False):
        ...
        repo = client.repo
        repo.authenticate('nathan', 'nathan')

        # url = 'http://cs-people.bu.edu/lapets/591/examples/lost.json'
        url = 'http://datamechanics.io/data/generalElectionData.csv'
        response = urllib.request.urlopen(url).read().decode("utf-8")
        transformedData = generalElection.removePeriods(generalElection.getAggByTownResponse(response))
        # repo.dropCollection("lost")
        repo.createCollection("nathan.aggGeneralData")
        #need to remove periods

        repo['nathan.aggGeneralData'].insert(transformedData)
        repo['nathan.aggGeneralData'].metadata({'complete': True})
        print(repo['nathan.aggGeneralData'].metadata())
    ...
    def provenance(doc=prov.model.ProvDocument(), startTime=None, endTime=None):
        ...
        this_script = doc.agent('alg:nathan#transformGeneral',
        ...
        aggByTown = doc.entity('dat:nathan#aggGeneralData',
