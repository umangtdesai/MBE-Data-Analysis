...
import uuid
import json
# from simon.builtin import project

class transformTotalVoter(dml.Algorithm):
    contributor = 'simon'
    reads = ['simon.democratic_primary', 'simon.republican_primary', 'simon.greenrainbow_primary',
             'simon.mapping']
    # writes = ['simon.three_parties_blanksandvotes_c_w_p',
    writes = ['simon.democratic_county_vote_stat',
              'simon.republican_county_vote_stat',
              'simon.greenrainbow_county_vote_stat']
    ...
    def execute(trial=False):
        ...
        repo = client.repo
        repo.authenticate('simon', 'simon')

        democratic = repo['simon.democratic_primary'].find()
        republican = repo['simon.republican_primary'].find()
        greenrainbow = repo['simon.greenrainbow_primary'].find()
        mapping = repo['simon.mapping'].find()
        ...
        # repo['simon.three_parties_blanksandvotes_c_w_p'].insert_many(projection_with_three)
        # repo['simon.three_parties_blanksandvotes_c_w_p'].metadata({'complete': True})
        ...
        repo['simon.democratic_county_vote_stat'].insert_many(json.loads(json.dumps(out_file1)))
        repo['simon.democratic_county_vote_stat'].metadata({'complete': True})

        repo.dropCollection('republican_county_vote_stat')
        repo.createCollection('republican_county_vote_stat')
        repo['simon.republican_county_vote_stat'].insert_many(json.loads(json.dumps(out_file2)))
        repo['simon.republican_county_vote_stat'].metadata({'complete': True})

        repo.dropCollection('greenrainbow_county_vote_stat')
        repo.createCollection('greenrainbow_county_vote_stat')
        repo['simon.greenrainbow_county_vote_stat'].insert_many(json.loads(json.dumps(out_file3)))
        repo['simon.greenrainbow_county_vote_stat'].metadata({'complete': True})
    ...
    def provenance(doc=prov.model.ProvDocument(), startTime=None, endTime=None):
        ...
        this_script = doc.agent('alg:simon#transformTotalVoter',
        ...
        democratic_vote_stat_by_county = doc.entity('dat:simon#democratic_county_vote_stat',
                                {prov.model.PROV_LABEL: 'democratic vote stat by county', prov.model.PROV_TYPE: 'ont:DataSet'})
        republican_vote_stat_by_county = doc.entity('dat:simon#republican_county_vote_stat',
                                {prov.model.PROV_LABEL: 'republican vote stat by county', prov.model.PROV_TYPE: 'ont:DataSet'})
        greenrainbow_vote_stat_by_county = doc.entity('dat:simon#greenrainbow_county_vote_stat',
