...
class bed_bath_amount(dml.Algorithm):
    ...
    writes = ['ekmak_gzhou_kaylaipp_shen99.bed_bath_amount']
    ...
    def execute(trial=False):
        ...
        #create new database 
        repo.dropCollection("bed_bath_amount")
        repo.createCollection("bed_bath_amount")

        #add to database in {street: count} form 
        for t in avgagg: 
            repo['ekmak_gzhou_kaylaipp_shen99.bed_bath_amount'].insert_one({t[0]:t[1]})
    ...
    def provenance(doc = prov.model.ProvDocument(), startTime = None, endTime = None):
        ...
        this_agent = doc.agent('alg:ekmak_gzhou_kaylaipp_shen99#bed_bath_amount',
		                      	{prov.model.PROV_TYPE: prov.model.PROV['SoftwareAgent'], 'ont:Extension': 'py'})

        this_entity = doc.entity('dat:ekmak_gzhou_kaylaipp_shen99#bed_bath_amount',
...
bed_bath_amount.execute()
print('generating num houses per street provenance...')
print('')
doc = bed_bath_amount.provenance()
print(doc.get_provn())
print(json.dumps(json.loads(doc.serialize()), indent=4))
