...
class getData (dml.Algorithm):
    ...
    reads = []
    writes = ['alice_bob.bostonEthnicities']
    ...
    def execute(trial = False):
        ...
        #for the data set boston_Ethnicities
        url = 'http://datamechanics.io/data/dixyTW_veeyn/race_ethnicity.json'
        response = urllib.request.urlopen(url).read().decode("utf-8")
        #r = [ json.loads(json.dumps(d)) for d in json.DictReader(StringIO(response))]
        r = json.loads(response)
        s = json.dumps(r, sort_keys=True, indent=2)
        repo.dropCollection("bostonEthnicities")
        repo.createCollection("bostonEthnicities")
        repo['alice_bob.bostonEthnicities'].insert_one(r)
        repo['alice_bob.bostonEthnicities'].metadata({'complete':True})
        print(repo['alice_bob.bostonEthnicities'].metadata())
